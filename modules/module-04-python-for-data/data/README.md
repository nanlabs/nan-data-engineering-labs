...
│   ├── orders.json           # 50,000 orders (nested)
│   ├── products.parquet      # 500 productos
│   ├── transactions.csv      # 100,000 transacciones
│   └── user_activity.json    # 20,000 eventos (nested)
├── processed/                # Clean data (generated by exercises)
│   └── (vacio - aqui guardaras tus resultados)
├── schemas/                  # Schemas JSON con documentation
...
**Size**: ~50 MB
**Encoding**: UTF-8  
**Structure**: Nested (items, shipping_address)
...
# Leer Parquet (mucho mas quick que CSV para files grandes)
...
**Size**: ~15 MB
**Encoding**: UTF-8  
**Structure**: Nested (device, location)
...
4. **Precios/stock negativos en products**: Data imposibles
...
```bash
# Ver tamanos de files
ls -lh data/raw/

# Contar lineas en CSV
...
| Formato | Advantages | Desventajas | Mejor Para |
|---------|----------|-------------|------------|
...
2. **Limpiar**: Implementa pipeline de limpieza (Exercise 03)
3. **Transformar**: Practica operaciones con pandas (Exercise 04)
4. **Validate**: Escribe tests para validate calidad (Exercise 06)
5. **Analizar**: Genera insights de negocio (Exercise 05)
