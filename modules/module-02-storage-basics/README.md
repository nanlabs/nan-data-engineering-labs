...
1. Generate sample data: `cd data/sample && bash generate_users.sh && cd ../..`
1. Read `theory/concepts.md` for fundamental understanding
1. Review `theory/architecture.md` for AWS architecture patterns
1. Configure infrastructure: `bash scripts/setup.sh`
1. Complete exercises in order (01 to 06)
...
1. **Exercise 02**: File Format Conversion - Convert between CSV, JSON, Parquet, Avro with performance benchmarking
1. **Exercise 03**: Partitioning Strategies - Implement Hive style partitioning (date, geography, hybrid) to optimize queries
1. **Exercise 04**: Compression Optimization - Compare Snappy, Gzip, LZ4, Zstd compression algorithms for Parquet
1. **Exercise 05**: Schema Evolution - Add columns, handle backward/forward compatibility in Parquet files
1. **Exercise 06**: Integration with Glue Catalog - Register datasets, create crawlers, query with Athena
