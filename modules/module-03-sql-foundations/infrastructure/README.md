...
1. Crea contenedor llamado `sql-foundations-postgres`
1. Crea volumen persistente para datos
1. Ejecuta `init.sql` para crear esquema y datos de muestra
1. Configura ajustes de performance
...
1. Descomentar volumen en `docker-compose.yml`:

   ```yaml
   - ./postgresql.conf:/etc/postgresql/postgresql.conf:ro
   ```

1. Agregar al comando:
...
1. Montar en contenedor

1. Agregar a `postgresql.conf`:
...
1. Verify connection: Try with `psql` from inside the container
1. Review section [Troubleshooting](#troubleshooting)
1. See `theory/architecture.md` for query optimization tips
1. Consult PostgreSQL documentation for specific errors
